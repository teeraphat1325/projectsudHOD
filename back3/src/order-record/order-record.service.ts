...
export class OrderRecordService {
  ...
  async create(
    ...
    // Validate that the staffName matches the user's name if userId is provided
    if (createOrderRecordDto.userId) {
      const user = await this.orderRecordRepository.manager.findOne(User, {
        where: { id: createOrderRecordDto.userId },
      });

      if (!user) {
        throw new NotFoundException(
          `User with ID ${createOrderRecordDto.userId} not found`,
        );
      }

      if (user.name !== createOrderRecordDto.staffName) {
        throw new NotFoundException(
          `Staff name '${createOrderRecordDto.staffName}' does not match with the user name '${user.name}'`,
        );
      }
    }
    ...
    const newOrderRecord = this.orderRecordRepository.create({
      ...
      // Only default to 1 if userId is not provided (undefined or null)
      userId:
        createOrderRecordDto.userId !== undefined
          ? createOrderRecordDto.userId
          : 1,
      orderDate: new Date(),
      status: 'pending',
    });
